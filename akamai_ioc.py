...
def create_resilient_session(client_token, client_secret, access_token):
    ...
    session.auth = EdgeGridAuth(
        client_token=client_token, client_secret=client_secret, access_token=access_token
    )
...
class APIAKAOpenParser:
    ...
    def parse_domain(self, rrecord):
        ...
        for k, _v in q.items():
            ...
            if k == "registrantName":
                aka_object.add_attribute(
                    "Registrant name", type="whois-registrant-name", value=str(q[k])
                )
            ...
            if k == "registrantEmails":
                aka_object.add_attribute(
                    "Registrant Emails", type="whois-registrant-email", value=str(q[k])
                )
            ...
            if k == "createdDate":
                aka_object.add_attribute(
                    "first-seen", disable_correlation=True, **{"type": "datetime", "value": q[k]}
                )
            if k == "lastModifiedDate":
                aka_object.add_attribute(
                    "last-seen", disable_correlation=True, **{"type": "datetime", "value": q[k]}
                )
                ...
                for item in threatIN:
                    if item["threatId"] != tmpI:
                        ...
                        addresult = session.get(
                            urljoin(
                                self.baseurl,
                                "/etp-report/v3/configs/"
                                + str(self.configID)
                                + "/threats/threat-meta?threatId="
                                + str(item["threatId"]),
                            ),
                            timeout=API_TIMEOUT,
                        )
                        ...
                        if not is_valid:
                            log.warning(
                                f"Failed to get threat metadata for {item['threatId']}: {error_msg}"
                            )
                            continue
                        threatInfo = (
                            "\nThreat Name: "
                        ...
                        try:
                            if d.get("familyName") and d.get("threatName"):
                                NEWTAGAPP = (
                                    "misp-galaxy:" + d["familyName"] + '="' + d["threatName"] + '"'
                                )
        ...
        aka_object.add_attribute(
            "Domain Threat Info", type="text", value=to_Enrich, Tag=tagval, disable_correlation=True
        )
    ...
    def _get_dns_info(self, rrecord):
        ...
        for dimension in dimensions:
            ...
            url = (
                f"/etp-report/v3/configs/{confID!s}"
            ...
            is_valid, error_msg, _result = validate_api_response(
                dns_response, f"DNS Activities ({dimension})"
            )
            if not is_valid:
...
def handler(q=False):
    ...
    mapping = {"domain": "parse_domain", "hostname": "parse_domain"}
